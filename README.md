...
Merge conflicts in `package-lock.json` should never be resolved manually. This repository includes a script to handle this safely.

**Usage:**

1.  Ensure `package.json` has no conflicts.
...
# Using with yarn
./scripts/fix-package-lock-conflict.sh --package-manager yarn

# Using with pnpm
./scripts/fix-package-lock-conflict.sh --package-manager pnpm

# Dry run mode (shows what it would do without making changes)
./scripts/fix-package-lock-conflict.sh --dry-run
```
