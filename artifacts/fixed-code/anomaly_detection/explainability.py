...
class AdvancedExplainabilityEngine:
    ...
    def __init__(
        ...
        self.model = model
        self.training_data = pd.DataFrame(training_data, columns=feature_names)
    ...
    def _setup_shap_explainer(self):
        ...
        try:
            if hasattr(self.model, "forward"):
                ...
                background_tensor = torch.from_numpy(self.training_data.values).float()
                ...
                self.shap_explainer = shap.Explainer(self.model, self.training_data.values)
    ...
    def _setup_lime_explainer(self):
        ...
        try:
            lime_training_data = self.training_data.values.reshape(
