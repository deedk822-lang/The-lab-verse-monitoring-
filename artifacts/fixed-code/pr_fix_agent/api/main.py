...
# Properly initialize and use environment variables
...
async def metrics():
    # Ensure the environment variables are properly loaded and initialized
    if "REDIS_HOST" not in os.environ or "DB_URL" not in os.environ:
        logger.error("missing_environment_variables")
        return JSONResponse(status_code=500, content={"detail": "Missing required environment variables"})
