...
class Settings(BaseSettings):
    ...
    jwt_private_key_path: Path = Field(default=Path("/path/to/secrets/jwt-private-key.pem"))
    jwt_public_key_path: Path = Field(default=Path("/path/to/secrets/jwt-public-key.pem"))

    # Database
    database_url: str = Field(
        default="postgresql+psycopg2://prfixagent:your_secure_password@localhost:5432/pr_fix_agent",
        env="DATABASE_URL"
    )
    ...
    # Redis
    redis_url: str = Field(
        default="redis://your_secure_username:your_secure_password@localhost:6379/0",
        env="REDIS_URL"
    )
