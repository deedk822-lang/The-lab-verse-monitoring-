...
class AuditLogger:
    ...
    def __new__(cls, log_path: Path):
        if cls._instance is None:
            ...
            cls._instance.log_path.parent.mkdir(parents=True, exist_ok=True)

            # Get logger
            cls._instance.logger = logging.getLogger("audit")
            ...
            for handler in cls._instance.logger.handlers:
                if isinstance(handler, logging.FileHandler):
                    ...
                    if hasattr(handler, 'baseFilename') and \
                       handler.baseFilename == str(cls._instance.log_path.resolve()):
