...
class OllamaBenchmarker:
    ...
    def query_timed(
        self, model: str, prompt: str, temperature: float = 0.2
    ) -> Tuple[str, float, int]:
    ...
    def run_code_generation_test(
        self, model: str, test_name: str, prompt: str, expected: List[str]
    ) -> BenchmarkMetrics:
    ...
    def run_benchmark_suite(self, model: str) -> List[BenchmarkMetrics]:
        ...
        m = self.run_error_analysis_test(
            model, "import_error_analysis", "ImportError: No module named 'requests'"
        )
        metrics.append(m)
        print(f"{'✓' if m.success else '✗'} ({m.response_time:.2f}s, Q:{m.quality_score:.2f})")
        ...
        m = self.run_error_analysis_test(
            model, "syntax_error_analysis", "SyntaxError: invalid syntax at line 10"
        )
        metrics.append(m)
        print(f"{'✓' if m.success else '✗'} ({m.response_time:.2f}s, Q:{m.quality_score:.2f})")
        ...
        m = self.run_error_analysis_test(
            model, "file_error_analysis", 'Error: File "config.py" not found'
        )
        metrics.append(m)
        print(f"{'✓' if m.success else '✗'} ({m.response_time:.2f}s, Q:{m.quality_score:.2f})")
    ...
    def print_performance_summary(self, perf: ModelPerformance):
        ...
        print(f"{'=' * 70}")
        print(
            f"Tests: {perf.total_tests} total, {perf.passed_tests} passed, {perf.failed_tests} failed"
        )
        print(f"Pass Rate: {perf.pass_rate() * 100:.1f}%")
        print(
            f"Response Time: {perf.avg_response_time:.2f}s avg, {perf.p95_response_time:.2f}s P95"
        )
        print(f"Quality Score: {perf.avg_quality_score:.2f}/1.00")
        print(
            f"Tokens: {perf.total_tokens} total, {perf.total_tokens / perf.total_tests:.0f} avg/test"
        )
    ...
    def compare_models(self, models: List[str]) -> Dict:
        ...
        for model in models:
            try:
                ...
                results[model] = {"metrics": metrics, "performance": perf}
            except Exception as e:
    ...
    def print_comparison_table(self, results: Dict):
        ...
        sorted_models = sorted(
            results.items(), key=lambda x: x[1]["performance"].pass_rate(), reverse=True
        )
        ...
        for model, data in sorted_models:
            ...
            print(
                f"{model:<20} {perf.pass_rate() * 100:>5.1f}%    {perf.avg_response_time:>6.2f}s    {perf.avg_quality_score:>5.2f}/1.0  {grade}  {winner}"
            )
...
class ProgressTracker:
    ...
    def print_progress_report(self, model: str):
        ...
        if len(history) >= 2:
            ...
            print("\nChanges:")
            print(
                f"  Pass Rate: {first.pass_rate() * 100:.1f}% → {latest.pass_rate() * 100:.1f}% ({pass_rate_change:+.1f}%)"
            )
            print(
                f"  Avg Time: {first.avg_response_time:.2f}s → {latest.avg_response_time:.2f}s ({time_change:+.2f}s)"
            )
            print(
                f"  Quality: {first.avg_quality_score:.2f} → {latest.avg_quality_score:.2f} ({quality_change:+.2f})"
            )
        ...
        for h in history[-10:]:  # Last 10
            print(
                f"{h.timestamp[:10]:<12} {h.pass_rate() * 100:>5.1f}%    {h.avg_response_time:>6.2f}s    {h.avg_quality_score:>5.2f}"
            )
...
def main():
    ...
    # Single model benchmark
        ...
        if args.output:
            output_data = {"performance": perf.to_dict(), "metrics": [m.to_dict() for m in metrics]}
            with open(args.output, "w") as f:
                json.dump(output_data, f, indent=2)
