    ...
    if (urgentOnly) {
      filtered = filtered.filter((deal) => {
        ...
        const daysUntilExpiry =
          (new Date(deal.expiry).getTime() - Date.now()) / (1000 * 60 * 60 * 24);
    ...
    <Card className="flex flex-col">
      ...
      <CardContent className="flex-grow">
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          ...
          {deal.creditUSD && (
            <Badge variant="secondary" className="flex items-center gap-1">
              <DollarSign className="h-4 w-4" />${deal.creditUSD}
            </Badge>
          )}
...
function DealAnalytics({ deals }: { deals: Deal[] }) {
  ...
  const averageScore =
    deals.length > 0 ? deals.reduce((sum, deal) => sum + deal.score, 0) / deals.length : 0;
  const averageCredit =
    deals.length > 0
      ? deals.reduce((sum, deal) => sum + (deal.creditUSD || 0), 0) / deals.length
      : 0;
  const averageTrial =
    deals.length > 0
      ? deals.reduce((sum, deal) => sum + (deal.trialDays || 0), 0) / deals.length
      : 0;
        ...
        <CardContent className="flex flex-wrap gap-2">
          {Object.entries(categoryCounts).map(([category, count]) => (
            <Badge key={category} variant="outline">
              {category}:<span className="font-semibold ml-1">{count}</span>
            </Badge>
          ))}
        </CardContent>
      </Card>
