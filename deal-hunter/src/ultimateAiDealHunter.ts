...
function computeScore(o: Offer): number {
  ...
  if (o.expiry) {
    ...
    if (daysUntilExpiry < 3)
      urgencyScore = 20; // High urgency for expiring soon
    else if (daysUntilExpiry < 7)
      urgencyScore = 10; // Medium urgency
...
async function extractWithDOM(
  content: string,
  source: { name: string; category: string; reliability: number }
): Promise<any[]> {
  ...
  for (const section of pricingSections) {
    ...
    if (!/(free trial|free credits?|promo|discount|coupon|credit|save)/i.test(textContent))
      continue;
    ...
    const discountPercentage =
      originalPrice && usd ? Math.round((1 - usd / originalPrice) * 100) : undefined;
              ...
              if (history) {
                ...
                if (
                  ...
                  !history.priceHistory.find(
                    (p: { price: number | undefined }) => p.price === offer.originalPrice
                  )
                ) {
...
export async function runUltimateDealHunter() {
  ...
  // Score and rank
  const top = deduplicatedOffers.sort((a, b) => b.score - a.score).slice(0, CONFIG.MAX_RESULTS);
  ...
  for (const offer of top) {
    ...
    if (history) {
      ...
      history.averageScore =
        (history.averageScore * (history.occurrenceCount - 1) + offer.score) /
  ...
  console.log(
    `✅ Found ${allOffers.length} offers, deduplicated to ${deduplicatedOffers.length}, top ${top.length} sent to notifications`
  );
...
async function sendDiscordNotification(offers: Offer[]) {
  const embeds = offers.map((o) => ({
    ...
    fields: [
      ...(o.trialDays
        ? [{ name: 'Trial Period', value: `${o.trialDays} days`, inline: true }]
        : []),
      ...(o.creditUSD ? [{ name: 'Free Credits', value: `$${o.creditUSD}`, inline: true }] : []),
      ...(o.discountPercentage
        ? [{ name: 'Discount', value: `${o.discountPercentage}%`, inline: true }]
        : []),
      ...(o.coupon ? [{ name: 'Coupon Code', value: `\`${o.coupon}\``, inline: true }] : []),
      ...(o.expiry
        ? [{ name: 'Expires', value: formatISO(new Date(o.expiry)), inline: true }]
        : []),
      ...(o.popularity
        ? [{ name: 'Popularity', value: o.popularity.toString(), inline: true }]
        : [])
    ].filter(Boolean),
...
async function sendSMSNotification(offers: Offer[]) {
  const message =
    `🚀 URGENT AI DEALS EXPIRING SOON:\n` +
    offers
      .map(
        (o) =>
          `${o.title} (${o.provider}): ${o.coupon ? `Code: ${o.coupon}` : `${o.creditUSD ? `$${o.creditUSD} credit` : `${o.trialDays} days trial`}`}`
      )
      .join('\n');
