...
async def main():
    ...
    if len(sys.argv) < 3 or len(sys.argv) > 5:
        print(
            'Usage: python rainmaker_cli.py <task_type> "<context>" [<output_filename>] [<model>]'
        )
        print(
            'Example: python rainmaker_cli.py coding_task "Calculate the 1000th Fibonacci number" fib.py kimimodel'
        )
        sys.exit(1)
    ...
    orchestrator = RainmakerOrchestrator(
        workspace_path="./ai_workspace", config_file="rainmaker_orchestrator/.env"
    )
