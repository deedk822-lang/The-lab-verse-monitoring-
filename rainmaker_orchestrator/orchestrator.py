...
class RainmakerOrchestrator:
    ...
    async def _call_judge(self, judge_role: str, context: str) -> dict[str, Any]:
        ...
        if zai_key:
            api_key: str = zai_key
            api_base_opt: str | None = self.config.get("ZAI_API_BASE")
            api_base: str = api_base_opt if api_base_opt else "https://api.z.ai/api/paas/v4"
            model: str = "glm-4.7"
        elif mistral_key:
            api_key = mistral_key
            api_base_opt = self.config.get("MISTRAL_API_BASE")
            api_base = api_base_opt if api_base_opt else "https://api.mistral.ai/v1"
    ...
    async def _run_self_healing(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        fail_result: dict[str, Any] = {
            "status": "failed",
            "message": "Max retries exceeded for self-healing",
        }
        return fail_result
