...
const buildServer = (logger = true): FastifyInstance => {
  ...
  server.post('/v1/scout', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    marketDataBreaker.fallback(() =>
      Promise.resolve({ data: { query, prices: [99, 99, 99] }, stale: true })
    );
    ...
    const pricingV2Enabled = await featureFlags.getBooleanValue('pricingV2', false, {
      targetingKey: tenant
    });

    if (pricingV2Enabled) {
      price = price * 1.2; // 20% uplift experiment
