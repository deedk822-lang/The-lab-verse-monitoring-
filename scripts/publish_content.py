...
class AyrshareTool:
    ...
    def post_to_socials(self, text: str, image_url: Optional[str] = None) -> str:
        ...
        payload = {"post": text, "platforms": ["twitter", "linkedin", "facebook"]}
...
class MailChimpTool:
    def __init__(self):
        ...
        if not all([self.api_key, self.server, self.list_id]):
            logger.warning(
                "MailChimp credentials incomplete - newsletter disabled (need MAILCHIMP_MARKETING__KEY, MAILCHIMP_SERVER_PREFIX, MAILCHIMP_LIST_ID)"
            )
            return
    ...
    def send_campaign(self, subject: str, html_content: str, plain_text: str) -> str:
        ...
        try:
            ...
            logger.warning(
                f"MAILCHIMP: Campaign '{campaign_id}' is ready. Sending is disabled in this script for safety."
            )
...
class WordPressTool:
    ...
    def publish_article(self, title: str, content: str, tags: list = None) -> str:
        ...
        try:
            r = requests.post(
                url, json=payload, auth=(self.username, self.app_password), timeout=20
            )
            r.raise_for_status()
            post_data = r.json()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Publish content from an HTML file to multiple channels."
    )
    parser.add_argument(
        "html_filepath", type=str, help="The path to the HTML file to be published."
    )
