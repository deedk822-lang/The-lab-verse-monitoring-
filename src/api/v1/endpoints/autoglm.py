...
async def generate_with_glm(
    request: GLMGenerateRequest, current_user: User = Depends(get_current_user)
):
    """
...
async def autoglm_secure_content(
    request: AutoGLMSecureContentRequest, current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        async with create_autoglm_orchestrator() as autoglm:
            secure_content = await autoglm.generate_secure_content(
                request.content_type, request.context
            )
...
async def autoglm_health_check(current_user: User = Depends(get_current_user)):
    ...
    if current_user.has_permission("glm") and settings.ZHIPU_API_KEY:
        try:
            async with create_glm_integration() as glm:
                test_response = await glm.generate_text(
                    "Hello, are you working?", {"max_tokens": 10}
                )
    ...
    if (
        current_user.has_permission("autoglm")
        and settings.ZHIPU_API_KEY
        ...
        try:
            ...
            health_status["services"]["autoglm"] = {
                "status": "error",
