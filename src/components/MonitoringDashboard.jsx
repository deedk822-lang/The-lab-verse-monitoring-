    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold text-gray-900">🔍 Lab Verse Monitoring Dashboard</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-3">
              <MemoryBar
                label="Heap Used"
                value={overview?.system?.memory?.heapUsed}
                max={overview?.system?.memory?.heapTotal}
                color="blue"
              />
              <MemoryBar
                label="RSS"
                value={overview?.system?.memory?.rss}
                max={overview?.system?.memory?.heapTotal * 1.5}
                color="purple"
              />
              <MemoryBar
                label="External"
                value={overview?.system?.memory?.external}
                max={overview?.system?.memory?.heapTotal * 0.5}
                color="green"
              />
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <ResponsiveContainer width="100%" height={150}>
              <PieChart>
                <Pie
                  data={Object.entries(costs?.byService || {}).map(([name, value]) => ({
                    name,
                    value
                  }))}
                  dataKey="value"
            ...
            {synthetic?.endpoints?.map((endpoint) => (
              ...
              >
                ...
                <div className="text-right">
                  <p
                    className={`font-bold ${endpoint.success ? 'text-green-600' : 'text-red-600'}`}
                  >
