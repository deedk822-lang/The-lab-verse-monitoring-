...
export const providers = {
  ...
  connect_ai_mcp: {
    baseURL: 'https://connect.ai-mcp.com/v1',
    authHeader: () => ({ authorization: `Bearer ${process.env.CONNECT_AI_MCP_API_KEY}` }),
    list: () => ['socialpilot-mcp', 'unito-mcp', 'hf-mcp'],
    priority: 10,
    enabled: () => !!process.env.CONNECT_AI_MCP_API_KEY
  },
  unito: {
    baseURL: 'https://connect.ai-mcp.com/v1',
    authHeader: () => ({ authorization: `Bearer ${process.env.CONNECT_AI_MCP_API_KEY}` }),
  ...
  socialpilot: {
    ...
    authHeader: () => ({
      ...
      'x-access-token': process.env.SOCIALPILOT_ACCESS_TOKEN
    }),
...
export function pickProvider(requestedModel, registry = providers) {
  ...
  for (const [name, meta] of enabled)
    if (meta.list().includes(requestedModel))
      return { provider: meta, upstreamModel: requestedModel, meta: { name, ...meta } };
  ...
  for (const [prefix, key] of Object.entries(familyMap)) {
    if (requestedModel.startsWith(prefix)) {
      ...
      if (meta?.enabled())
        return { provider: meta, upstreamModel: meta.list()[0], meta: { name: key, ...meta } };
    }
  }
