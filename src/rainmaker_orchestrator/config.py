...
class ConfigManager:
    ...
    def _load_config(self) -> Dict[str, str]:
        ...
        config.update({
            ...
            'ZAI_API_KEY': os.getenv('ZAI_API_KEY') or os.getenv('zai-api-key'), # Handle both casing variants
        })

        # Filter out None values
        loaded_config = {k: v for k, v in config.items() if v is not None}
        
        # Critical Secret Warning (Prevents Runtime Failures in Vercel)
        if not loaded_config.get('ZAI_API_KEY'):
            print("⚠️ WARNING: ZAI_API_KEY (zai-api-key) is missing. System will run in LIMITED mode.")
