...
class MCPService {
  ...
  async generateWithClaude(params) {
    try {
      ...
      const response = await axios.post(`${this.claude.baseURL}/v1/messages`, payload, {
        headers: {
  ...
  async generateWithMistral(params) {
    try {
      ...
      const response = await axios.post(`${this.mistral.baseURL}/chat/completions`, payload, {
        headers: {
  ...
  async generateContent(params) {
    try {
      const { preferredProvider = 'claude', useFailover = true, ...otherParams } = params;
  ...
  async generateSupportResponse(params) {
    try {
      ...
      systemPrompt +=
        "Always aim to resolve the customer's issue completely and offer additional help if needed.";
  ...
  async analyzeFinancialData(params) {
    try {
      ...
      let systemPrompt =
        'You are a financial data analyst. Provide accurate, insightful analysis of financial data. ';
      systemPrompt +=
        'Focus on trends, patterns, and actionable insights. Present information clearly and professionally.';
    ...
    try {
      ...
      for (const language of targetLanguages) {
        ...
        if (culturalAdaptation) {
          prompt +=
            'Please adapt the content for the target culture and market, not just literal translation.';
        } else {
      ...
      logger.info('Multi-lingual content generation completed:', {
        targetLanguages,
        successful: Object.keys(results.translations).filter(
          (lang) => !results.translations[lang].startsWith('Translation failed')
        ).length
      });
