...
class ImageGenerator:
    ...
    def __init__(self):
        # ⚡ Bolt Optimization: Reuse session for connection pooling
    ...
    def generate_batch(self, prompts: List[str], style: str = "professional") -> List[Dict]:
        ...
        ⚡ Bolt Optimization: Uses ThreadPoolExecutor for parallel generation.
        """
        if not prompts:
            return []

        def _generate_single(prompt_tuple):
        ...
        # Use max 5 threads to avoid overwhelming APIs/local resources
        with ThreadPoolExecutor(max_workers=min(len(prompts), 5)) as executor:
            results = list(executor.map(_generate_single, enumerate(prompts)))
