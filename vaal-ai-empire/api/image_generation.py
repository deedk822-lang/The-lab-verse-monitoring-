...
class ImageGenerator:
    ...
    def generate_batch(self, prompts: List[str], style: str = "professional") -> List[Dict]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            # Keep track of futures to maintain order
            future_to_index = {
                executor.submit(self.generate, prompt, style): i
                for i, prompt in enumerate(prompts)
            }

            logger.info(f"Dispatched {len(prompts)} image generation tasks in parallel.")
            ...
            for future in concurrent.futures.as_completed(future_to_index):
                ...
                try:
                    ...
                    logger.info(f"Completed image {index + 1}/{len(prompts)}.")
                except Exception as e:
                    logger.error(f"Failed to generate image for prompt: {prompt}", exc_info=True)
                    results[index] = {
                        "image_url": self._create_placeholder(prompt),
                        "provider": "error",
                        "cost_usd": 0.0,
