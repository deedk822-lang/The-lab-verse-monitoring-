...
def test_optimization():
    ...
    if not OptimizedContentFactory:
        print("❌ OptimizedContentFactory not found. Skipping benchmark.")
        return False
    ...
    if hasattr(_get_providers, 'cache_clear'):
    ...
    is_fast = subsequent_time < 0.1 # Increased threshold slightly for environment variance

    print("-" * 60)
    print(f"Subsequent Instantiation Time: {subsequent_time:.4f}s")
    print(f"Is subsequent time negligible (< 0.1s)? {'Yes' if is_fast else 'No'}")
    print("=" * 60)
