...
class ContentFactory:
    ...
    def _ensure_providers(self) -> None:
        ...
        if self._providers is None:
            self._providers, self._image_generator, self._multimodal_provider = (
                _get_cached_providers()
            )
    ...
    @property
    def image_generator(self) -> Any | None:
        self._ensure_providers()
        return self._image_generator

    @property
    def multimodal_provider(self) -> Any | None:
        self._ensure_providers()
    ...
    def generate_social_pack(
        ...
        ⚡ Bolt Optimization: Parallelizes text and image generation.
        """
        ...
        try:
            ...
            # ⚡ Bolt Optimization: Run text and image generation in parallel
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                # Start text generation
                posts_future = executor.submit(self.generate_content, posts_prompt, max_tokens=2000)

                # Start image generation if generator available
                images_future = None
