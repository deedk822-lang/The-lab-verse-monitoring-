...
class ContentFactory:
    ...
    Providers are initialized only when they are first requested.
    """

    def __init__(self, db=None):
        self.db = db
        self._providers_cache: Dict[str, Optional[Any]] = {}
        self.provider_names = ["cohere", "groq", "mistral", "huggingface", "kimi"]
        # Special providers are handled by properties to load them on first access
        self._image_generator: Optional[Any] = None
        self._multimodal_provider: Optional[Any] = None
        self._image_generator_loaded = False
        self._multimodal_provider_loaded = False

    def _get_provider(self, provider_name: str) -> Optional[Any]:
        """Lazy-loads a single text generation provider and caches the result."""
        if provider_name in self._providers_cache:
            return self._providers_cache[provider_name]

        provider = None
        try:
            logger.info(f"Initializing {provider_name.capitalize()} provider...")
            if provider_name == "cohere":
                ...
                # This case should ideally not be hit if provider_names is accurate
                raise ImportError(f"Unknown provider name: {provider_name}")

            logger.info(
                f"✅ {provider_name.capitalize()} provider initialized successfully"
            )
            self._providers_cache[provider_name] = provider

        except (ImportError, ValueError) as e:
            logger.warning(
                f"⚠️  {provider_name.capitalize()} provider unavailable: {e}"
            )
    ...
    def image_generator(self) -> Optional[Any]:
        """Lazy-loads the image generation provider upon first access."""
        if not self._image_generator_loaded:
            logger.info("Initializing Image Generation provider...")
            try:
                from api.image_generation import BusinessImageGenerator

                self._image_generator = BusinessImageGenerator()
                logger.info("✅ Image generation provider initialized")
            except Exception as e:
                logger.warning(f"⚠️  Image generation disabled: {e}")
                self._image_generator = None
            self._image_generator_loaded = True
        return self._image_generator

    @property
    def multimodal_provider(self) -> Optional[Any]:
        """Lazy-loads the multimodal (Aya Vision) provider upon first access."""
        if not self._multimodal_provider_loaded:
            logger.info("Initializing Aya Vision multimodal provider...")
            try:
                from api.aya_vision import AyaVisionAPI

                self._multimodal_provider = AyaVisionAPI()
            ...
            self._multimodal_provider_loaded = True
    ...
    def generate_social_pack(
        ...
        try:
            ...
            images = []
            # Access the property to trigger lazy loading
            image_gen = self.image_generator
            if image_gen:
                try:
...
Generate all {days} emails now:"""
    ...
    def get_provider_status(self) -> Dict:
        status = {}
        # This will trigger lazy loading for each provider if not already loaded
