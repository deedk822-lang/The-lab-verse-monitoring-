    ...
    def generate_from_messages(self, messages: List[Dict[str, Any]], max_new_tokens: int = 300) -> Dict:
        ...
        for msg in messages:
            ...
            if isinstance(msg.get("content"), str):
                new_msg["content"] = sanitize_prompt(msg["content"])
            elif isinstance(msg.get("content"), list):
                new_content = []
                for item in msg["content"]:
                    if isinstance(item, dict) and item.get("type") == "text" and "text" in item:
                        new_item = item.copy()
                        new_item["text"] = sanitize_prompt(item["text"])
        ...
        try:
            ...
            outputs = self.pipe(sanitized_messages, max_new_tokens=max_new_tokens, return_full_text=False)
