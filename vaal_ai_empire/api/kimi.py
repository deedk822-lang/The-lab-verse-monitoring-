...
class KimiAPI:
    ...
    def __init__(self):
        self.client = OpenAI(
            base_url=os.getenv("KIMI_API_BASE", "http://localhost:8000/v1"), api_key=os.getenv("KIMI_API_KEY", "EMPTY")
        )
        self.model = os.getenv("KIMI_MODEL", "Qwen/Qwen-72B-Instruct")

    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
    def generate_content(
        self, prompt: str, system_prompt: str = "You are Kimi Linear, an expert AI assistant."
    ) -> Dict:
        """
