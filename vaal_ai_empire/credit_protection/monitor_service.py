                ...
                if max_usage_percent >= self.critical_threshold and not self.critical_sent:
                    ...
                    if max_usage_percent >= 95:
                        ...
                        await self.alert_service.send_alert(
                            ...
                            message=(
                                f"Circuit breaker activated due to high usage ({max_usage_percent:.1f}%).\n"
...
Current Usage:
- Requests: {daily['requests']} / {daily['limits']['requests']} ({daily['usage_percent']['requests']:.1f}%)
- Tokens: {daily['tokens']:,} / {daily['limits']['tokens']:,} ({daily['usage_percent']['tokens']:.1f}%)
- Cost: ${daily['cost_usd']:.4f} / ${daily['limits']['cost_usd']:.2f} ({daily['usage_percent']['cost']:.1f}%)
...
Please monitor your usage to avoid hitting daily limits.
        ...
        logger.warning(f"Usage warning sent: {daily['usage_percent']['requests']:.1f}% of daily limit")
...
3. Upgrading to a higher tier
        ...
        logger.error(f"Critical usage alert sent: {daily['usage_percent']['requests']:.1f}% of daily limit")
